namespace LWhisper.Core.Models
{
    ...
    public class StreamingSettings
    {
        ...
        /// <summary>
        /// Порог МАКСИМАЛЬНОЙ амплитуды сегмента (доля от full-scale, не дБ): ниже — сегмент
        /// считается тишиной/фоновым шумом и не отправляется в распознавание.
        ///
        /// Замер 2026-08-11 по 8 дням боевых логов: гейт сработал 46 раз, медиана пика 0.0038
        /// (честная тишина), но 10 случаев имели пик ≥ 0.015, и один из них содержал реальную,
        /// просто тихую речь. Снижать значение вслепую нельзя: гейт — единственный слой, который
        /// отсекает шум ДО вызова движка, а фильтры после распознавания ловят не всякую короткую
        /// галлюцинацию. Поэтому параметр вынесен в настройки, но дефолт оставлен прежним.
        /// </summary>
        public double SilenceMaxAmplitudeThreshold { get; set; } = 0.03;
    }
}
